...
interface MarkItemSoldFormProps {
    onSubmit: (item: string) => Promise<void>;
    
}

const MarkItemSoldForm: React.FC<MarkItemSoldFormProps> = ({ onSubmit }) => {
    const [item, setItem] = useState('');
    const [snackbarVisible, setSnackbarVisible] = useState(false);
    const [snackbarMessage, setSnackbarMessage] = useState('');

    const handleMarkItemSoldSubmit = async () => {
        try {
            await onSubmit(item);
    ...
    return (
        <View style={styles.container}>
            ...
            <Pressable style={styles.button} onPress={handleMarkItemSoldSubmit} > 
...
export default MarkItemSoldForm;
